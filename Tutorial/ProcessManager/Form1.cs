...
namespace ProcessManager
{
    public partial class Form1 : Form
    {
        ...
        private void ProcessView()
        {
            try
            {
                ...
                foreach (var proc in Process.GetProcesses()) //모든 프로세스의 목록을 가져 오자
                {
                    ...
                    this.ProcessView1.Items.Add(lvt);
                    processCount++;
                }
            }
            catch { }
            this.ProcessCurrent.Text = "프로세스 : " + processCount.ToString() + "개";
        }
        ...
        private void getCPU_Info()
        {
            try
            {
                while (!bExit)
                {
                    ...
                    Thread.Sleep(1000);  //1초마다 status update
                }
            }
            catch { }
        }
        ...
        private void TotalView(int m, int n)
        {
            try
            {
                this.CPUCurrent.Text = "CPU 사용: " + m.ToString() + " %";
                this.MemoryCurrent.Text = "실제 메모리 : " + n.ToString() + " %";
            }
            catch { }
        }
        ...
        {
            int SelectRow = ProcessView1.SelectedItems[0].Index;
            //string lProcessName = ProcessView1.Items[SelectRow].SubItems[0].Text;
            Process[] process = new Process[1];
            process = Process.GetProcessesByName(ProcessView1.Items[SelectRow].SubItems[0].Text);
            process[0].Kill();
        }
    }
}
